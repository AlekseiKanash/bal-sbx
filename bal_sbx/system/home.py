...
class RealHomeLayout(HomeLayout):
    """Filesystem-backed home layout.

    The sandbox HOME lives under a system-owned parent (`/Users` on macOS,
    `/home` on Linux), so every mutation is routed through `PrivilegeBroker`.
    Read-only probes (`exists`, `workspace_link_target`) remain unprivileged
    since the created tree is mode 0755 and traversable by the caller.
    """
    ...
    def create(self, home: str, username: str) -> None:
        self._privilege.run_privileged(["mkdir", "-p", home])
        self._privilege.run_privileged(["chown", username, home])

    def destroy(self, home: str) -> None:
        if os.path.isdir(home):
            self._privilege.run_privileged(["rm", "-rf", home])
    ...
    def link_workspace(self, home: str, workspace: str) -> None:
        ...
        self._privilege.run_privileged(["mkdir", "-p", os.path.dirname(link)])
        self._privilege.run_privileged(["ln", "-sfn", workspace, link])
